...
/**
 *Submitted for verification at BscScan.com on 2021-07-16
 * TG = https://t.me/PuzzleChainToken
 * Whitepaper https://www.puzzlechain.com/puzzlepaper
*/
...
contract PuzzleChain is Context, IERC20, Ownable {
    ...
    string private _name = "PuzzleChain";
    string private _symbol = "PUZZLE";
    ...
    event SwapAndbuyback(
        uint256 tokensSwapped,
        uint256 ethReceived,
        uint256 tokensIntobuyback
    );
    ...
    function setbuybackAddress(address payable buyback) public onlyOwner {
    ...
    function showbuybackaddress() public view returns(address payable) {
    ...
    function _getTValues(uint256 tAmount) private view returns (uint256, uint256, uint256) {
        ...
        uint256 tLiquidity = calculateLiquidityPlusbuybackFee(tAmount);
    ...
    function calculateLiquidityPlusbuybackFee(uint256 _amount) private view returns (uint256) {
        return _amount.mul(_liquidityFee + _buybackFee + _marketingAndDevBudget).div(
            10**2
        );
    }
    ...
    function removeAllFee() private {
        if(_taxFee == 0 && _liquidityFee == 0 && _marketingAndDevBudget == 0 && _buybackFee == 0) return;
    ...
    function swapAndLiquify(uint256 contractTokenBalance) private lockTheSwap {
        ...
        uint256 totalLiqFee = _marketingAndDevBudget + _liquidityFee + _buybackFee;
        ...
        // calculate the portions of the liquidity to add to buyback
        uint256 buybackBalance = newBalance.div(totalLiqFee).mul(_buybackFee);
        uint256 buybackPortion = otherHalf.div(totalLiqFee).mul(_buybackFee);
        ...
        uint256 finalBalance = newBalance.sub(buybackBalance).sub(marketingBalance);
        uint256 finalHalf = otherHalf.sub(buybackPortion).sub(marketingPortion);
        
        
        
        (bool sent, bytes memory data) = _buybackAddress.call{value: buybackBalance}("");
        if(sent){
            _tokenTransfer(address(this), 0x0000000000000000000000000000000000000001, buybackPortion, false);
            emit Transfer(address(this), 0x0000000000000000000000000000000000000001, buybackPortion);
        } else {
            addLiquidity(buybackPortion, buybackBalance, _buybackAddress);
        }
    ...
    function addLiquidity(uint256 tokenAmount, uint256 ethAmount, address buyback) private {
